...
void insertionSortJumlahDescending(vector<Pertanyaan> &daftar) {
    int ukuran = (int)daftar.size();
...
void editPertanyaan(int index_pengguna) {
    ...
    if (daftar_index.empty()) { cout << "  Belum ada pertanyaan." << endl << endl; return; }
    ...
    if (nomor_pilihan < 1 || nomor_pilihan > (int)daftar_index.size()) {
        cout << "  Nomor ga valid." << endl << endl; return;
    }
...
void hapusPertanyaanUser(int index_pengguna) {
    ...
    if (daftar_index.empty()) { cout << "  Blom ada pertanyaan." << endl << endl; return; }
...
void lihatSemuaPertanyaan() {
    ...
    if (daftar_pertanyaan.empty()) { cout << "  Belum ada pertanyaan." << endl << endl; return; }

    vector<Pertanyaan> daftar_terurut = daftar_pertanyaan;
    insertionSortJumlahDescending(daftar_terurut);
...
void lihatDaftarPengguna() {
    if (daftar_pengguna.empty()) { cout << "  Belum ada pengguna." << endl << endl; return; }
...
void hapusPertanyaanAdmin() {
    if (daftar_pertanyaan.empty()) { cout << "  Blom ada pertanyaan." << endl << endl; return; }
...
void tambahJawabanKerang() {
    ...
    if (jawaban_baru == "0")          cout << "  Dibatalkan." << endl << endl;
    else if (jawaban_baru.empty())    cout << "  Jawaban ga boleh kosong." << endl << endl;
    else {
